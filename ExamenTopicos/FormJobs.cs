...
namespace ExamenTopicos
{
    ...
    public partial class FormJobs : Form
    {
        ...
        private void dgvPuestos_CellBeginEdit(object sender, DataGridViewCellCancelEventArgs e)
        {
            ...
            {
                MessageBox.Show($"Error al iniciar edición: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void dgvPuestos_RowValidating(object sender, DataGridViewCellCancelEventArgs e)
        {
            try
            {
                ...
                if (row.Cells["min_lvl"].Value == null || !int.TryParse(row.Cells["min_lvl"].Value.ToString(), out int newMinLvl))
                {
                ...
                if (row.Cells["max_lvl"].Value == null || !int.TryParse(row.Cells["max_lvl"].Value.ToString(), out int newMaxLvl))
                {
                ...
                if (originalData.JobDesc != newDescripcion || originalData.MinLvl != newMinLvl || originalData.MaxLvl != newMaxLvl)
                {
                    StringBuilder mensaje = new StringBuilder();
                    mensaje.AppendLine("¿Deseas guardar los siguientes cambios?\n");

                    if (originalData.JobDesc != newDescripcion)
                    {
                        mensaje.AppendLine($"• Descripción: '{originalData.JobDesc}' ➔ '{newDescripcion}'");
                    }
